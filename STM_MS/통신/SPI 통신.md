...
SPI는 SCK의 어느 엣지에서 데이터를 읽을지를 정해야 합니다.

먼저 용어를 이해해야 합니다.

## 유휴 상태란

통신을 하지 않고 있을 때 SCK가 유지되는 상태입니다.

```
CS가 High이고 전송하지 않는 상태
```

이때 SCK가 Low인지 High인지를 CPOL이 결정합니다.
...
첫 번째 엣지는 Rising edge입니다.

## CPOL = 1

유휴 상태에서 SCK가 High입니다.
...
CPHA는 어느 엣지에서 데이터를 샘플링할지를 정합니다.

## CPHA = 0

첫 번째 엣지에서 데이터를 읽습니다.

두 번째 엣지에서 다음 데이터를 준비합니다.

## CPHA = 1

첫 번째 엣지에서 데이터를 변경하거나 준비합니다.

두 번째 엣지에서 데이터를 읽습니다.

※ Master와 Slave의 CPOL/CPHA가 다르면 데이터가 깨짐(설정이 같아야함 + 데이터 길이 MSB/LSB First)
## 네 가지 모드
...
MOSI는 Rising edge 전에 안정되어 있어야 합니다.
...
- Prescaler(분주기 - 입력 클럭을 나눔)가 2일 때 수 MHz가 나오면 화면에서 보기 어려움(크게 맞춰서 SCK를 느리게 해야 안정적)
